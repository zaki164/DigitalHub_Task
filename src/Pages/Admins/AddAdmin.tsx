...
const AddAdmin = () => {
  ...
  const Schema = Yup.object().shape({
    ...
    mobile: Yup.string().required(t('Mobile telephone is required'))
      .matches(
        /^(5)([0-9]{8})$/,
  ...
  return (
    <motion.section
      ...
      <p className="page-header">{t("Add new admin")}</p>
      <form onSubmit={formik.handleSubmit} className="mt-5 md:mt-8" noValidate>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 md:gap-6">
          ...
          <div className="flex flex-col gap-2 md:gap-3 mb-3 md:mb-4">
            ...
            <div className="bg-whiteColor text-grayColor custom-rounded ps-4 flex items-center justify-between flex-wrap text-base-l">
              ...
              <div className="py-4 px-3">
                {!formik.values.ID_Photo ? t("Upload a photo") :
                  <div className="w-36">
                    <img className='object-contain max-w-full' src={URL.createObjectURL(formik.values.ID_Photo)} alt="ID Photo" />
                  </div>
                }
              </div>
              <button
          ...
          <div className="flex flex-col gap-2 md:gap-3 mb-3 md:mb-4">
            <label className="custom-label">{t("Mobile Telephone")}</label>
            <input type="text" className="custom-input" maxLength={9} placeholder={t("Enter Telephone")} {...formik.getFieldProps('mobile')} />
